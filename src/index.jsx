...
const Grid = (size) => {
  ...
  for (let x = 0; x < cells; x += 1) {
    for (let y = 0; y < cells; y += 1) {
      if (Math.random() > 0.5) {
        str += Square(CELL_SIZE, Math.random() > 0.5);
        str += ` m -.5 -.5`;
...
const Circle = (size) => {
  size = Math.random() > 0.5 ? size * 50 : size;
  const half = size / 2;
...
const Panel = (width, height, startX = 40, startY = 40) => {
  let pathString = "";
  ...
  const items = [Square, Grid, Plus, Circle];

  for (var x = 0; x < 2; x += 1) {
    // Draw verticals
    for (var y1 = 0; y1 < cellsY; y1 += 1) {
      if (Math.random() > 0.5) {
        const posX = x * width;
        const posY = y1 * SPACING_Y;
        pathString += ` M ${startX + posX} ${startY + posY}`;
        const shape = items[Math.floor(Math.random() * items.length)];
        const size = Math.floor(Math.random() * 5) * SIZE_INTERVAL;
...
const Box = ({ children }) => {
  ...
  return (
    <div ref={ref} style={{ 
        filter: "url(#filter)", position: "relative", height: "100%" }}>
      <canvas
        width="100%"
        height="100%"
...
function buildPath() {
  ...
  for (let x = 0; x < Math.floor(window.innerWidth / SPACING_X); x += 1) {
    for (let y = 0; y < Math.floor(window.innerHeight / SPACING_Y); y += 1) {
      if (Math.random() > 0.8) {
        // 3 in 10 chance
