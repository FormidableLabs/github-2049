...
import { Stats, Box as Box3D, Environment } from "drei";
...
function FlexSciFiPanel({ color = "#fff", children, ...rest }) {
  return (
    <FlexBox {...rest}>
      {(width, height) =>
        width &&
...
const SizedScene = React.memo(({ width, height }) => {
  const margin = 50;
  const halfMargin = margin / 2;
  return (
    <>
      <ambientLight args={[0xffffff, 0.5]} />
      <directionalLight args={["#fff", 0.5]} />
      <Background width={width} height={height} color="#fff" opacity={0.1} />
      <Flex
        scaleFactor={2}
        flexDirection="row"
        position={[0, height, 0]}
        ...
        padding={halfMargin}
      >
        <FlexBox width={300} height="auto" marginRight={halfMargin}>
          <FlexSciFiPanel
            padding={10}
            width="100%"
            flexShrink={1}
        ...
        <FlexSciFiPanel
          ...
          color="#fff"
        >
          <FlexBox marginTop={10} width="100%" height="100%" centerAnchor>
            {(width, height) => (
        ...
        <FlexBox width={200} height="auto">
          ...
          >
            <View
              ...
              marginBottom={10}
            >
              <TextBox font={bold} fontSize={12} color="#fff">
                COMMIT
              </TextBox>
              <TextBox font={regular} fontSize={12} color="#fff">
                {" ACTIVITY"}
              </TextBox>
            </View>
            <FlexBox width="100%">
...
function Scene() {
  const { size } = useThree();
  return <SizedScene width={size.width} height={size.height} />;
}
